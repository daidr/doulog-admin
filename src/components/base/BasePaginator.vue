...
function PageItemGroup() {
  ...
  // 渲染左侧省略号（中间至少有 1 页时）
  if (props.current > 3) {
    nodes.unshift(h(PageEllipsis, { min: 2, max: props.current - 2, current: props.current, onPageChange: (page: number) => emit('pageChange', page), key: 'left-ellipsis' }))
  }
  ...
  if (props.current < totalPageCount.value - 2) {
    nodes.push(h(PageEllipsis, { min: props.current + 2, max: totalPageCount.value - 1, current: props.current, onPageChange: (page: number) => emit('pageChange', page), key: 'right-ellipsis' }))
  }
...
<template>
  ...
  >
    <div
      v-if="showSwitch" class="page-item" :class="{
        disabled: props.current === 1,
      }" @click="emit('pageChange', props.current - 1)"
    >
      <div class="i-mingcute-left-line text-base" />
    </div>
    ...
    >
      <div class="i-mingcute-right-line text-base" />
    </div>
    <BaseSelect :options="sizerOptions" :model-value="size" class="important-text-sm" @update:model-value="emit('sizeChange', Number($event))" />
  </div>
</template>
