...
export const InsertBetween = Extension.create<NodeToolsOptions>({
  ...
  addProseMirrorPlugins() {
    ...
    const getTopLevelNodeDOMFromCoords = (view: EditorView, event: MouseEvent) => {
      let position = getTopLevelNodePositionFromCoords(
        view,
        event.clientX,
        event.clientY - 25
      );
      return position ? view.nodeDOM(position) : null;
    };

    const getTopLevelNodePositionFromCoords = (view: EditorView, x: number, y: number, after = false) => {
        ...
        props: {
          handleDOMEvents: {
            mousemove: (view, event) => {
              ...
              if (nodeDOM) {
                let rect = (nodeDOM as HTMLElement).getBoundingClientRect();
